...
fn test_01_missing_signer_check() {
    ...
    assert!(!findings.is_empty(), "01-missing-signer-check should have findings");
    let has_high = findings.iter().any(|f| f.severity == pinocchio_sentinel::rules::Severity::HIGH);
...
fn test_02_missing_owner_check() {
    ...
    assert!(!findings.is_empty(), "02-missing-owner-check should have findings");
}

#[test]
fn test_03_account_data_matching() {
    let findings = scan_benchmark("03-account-data-matching.rs");
    assert!(!findings.is_empty(), "03-account-data-matching should have findings");
}
...
fn test_all_benchmarks_have_findings() {
    ...
    for benchmark in benchmarks {
        ...
        assert!(!findings.is_empty(), "Benchmark {} should have findings", benchmark);
    }
}
...
fn test_findings_have_handler_names() {
    ...
    for finding in &findings {
        assert!(!finding.handler.is_empty(), "Finding should have handler name");
    }
}
...
fn test_findings_have_line_numbers() {
    let findings = scan_benchmark("01-missing-signer-check.rs");
    for finding in &findings {
        assert!(finding.line_number.is_some(), "Finding should have line number");
    }
}

#[test]
fn test_findings_have_valid_rule_ids() {
    let valid_rules = ["PS-001", "PS-002", "PS-003", "PS-004", "PS-005", "PS-006", "PS-007", "PS-008", "PS-009", "PS-010", "PS-011", "PS-012", "PS-013", "PS-014"];
    let findings = scan_benchmark("01-missing-signer-check.rs");
    for finding in &findings {
        assert!(valid_rules.contains(&finding.rule_id.as_str()), "Invalid rule ID: {}", finding.rule_id);
    }
}
